...
import ntu.nlp.service.CkipService;
...
public class ParallelClientMain{
	...
	private static final Logger LOG = LoggerFactory.getLogger(ParallelClientMain.class);
	...
	public static class  CallableTasker implements Callable<TermWrapper>{

		private Comment comment;
		
		public CallableTasker(Comment comment){
			this.comment = comment;
		}
		
		@Override
		public TermWrapper call() throws Exception {
			
			CkipService ckip = new CkipService();
			TermWrapper tw= null;
			
			LOG.info(Thread.currentThread().getName() + "is dealing with serial: " + comment.getSerial());
			if(comment != null){
				tw = ckip.ckipSplitTagByComment(IP, PORT, ACCOUNT, PWD, comment);
			}
			
			return tw;
		}
		
	}// End of class CallableTasker
